...
HttpConnection::HttpConnection(int f, Router *r)
    : fd(f), state(PS_RequestLine), req(0), expectedBody(0), currentChunkSize(0), headLike(false), shouldClose(false),
      headersSize(0), headerCount(0), _closed(false), expectContinue(false), sentContinue(false), router(r)
{
}
...
bool HttpConnection::parseStartLine(const std::string &line, std::string &method, std::string &target,
                                    std::string &version)
{

...
std::string Router::allowForPath(const std::string& path) const {
    ...
    for (std::map<std::string, std::map<std::string, HandlerFn> >::const_iterator mit = table.begin(); mit != table.end(); ++mit) {
        if (mit->second.find(path) != mit->second.end()) {
            std::string m = mit->first;
            for (size_t i = 0; i < m.size(); ++i) m[i] = std::toupper(static_cast<unsigned char>(m[i]));
            methods.push_back(m);
        }
    }
    std::string result;

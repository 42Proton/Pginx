    ...
    for (size_t i = 0; i < servers.size(); ++i) {
        ...
        if (!indexFiles.empty()) {
            ...
            for (size_t j = 0; j < indexFiles.size(); ++j) {
                ...
                if (j < indexFiles.size() - 1) std::cout << ", ";
            }
            std::cout << std::endl;
        }
        ...
        // Error pages
        const std::map<u_int16_t, std::string> &errorPages = server.getErrorPage(0) ? 
        ...
        if (!locations.empty()) {
            ...
            for (size_t j = 0; j < locations.size(); ++j) {
                ...
                switch (loc.getMatchType()) {
                    case EXACT: matchTypeStr = "= "; break;
                    case REGEX_CASE: matchTypeStr = "~ "; break;
                    case REGEX_ICASE: matchTypeStr = "~* "; break;
                    case PRIORITY_PREFIX: matchTypeStr = "^~ "; break;
                    case NAMED: matchTypeStr = "@ "; break;
                    case PREFIX: 
                    default: matchTypeStr = ""; break;
                }
                
                std::cout << "\\n    [Location: " << matchTypeStr << loc.getPath() << "]" << std::endl;
                ...
                if (!locIndexFiles.empty()) {
                    ...
                    for (size_t k = 0; k < locIndexFiles.size(); ++k) {
                        ...
                        if (k < locIndexFiles.size() - 1) std::cout << ", ";
                    }
                    std::cout << std::endl;
                }
